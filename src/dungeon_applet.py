...
from online_session.journal import JournalEntry, PlayerUndoJournal
...
def _generate_probabilistic_unique_id(prefix: str) -> str:
    ...
    machine_entropy = _machine_entropy_string()
...
class DungeonAppletWidget(QWidget):
    def __init__(self, parent=None) -> None:
        ...
        self._persistent_local_player_id: str = str(
            self._local_profile.get("player_id") or _generate_probabilistic_unique_id("player")
        )
        ...
        self._player_undo_journal = PlayerUndoJournal()
        self._view_mode = "dm"
    ...
    def _render_initiative_overlay(self) -> None:
        ...
        if is_player and displayed_player_rows == 0:
    ...
    def _on_client_hello_ack(self, player_id: str) -> None:
        ...
        self._append_chat_message("System", f"Joined as {self._local_player_name}", True)
        self._apply_online_permissions()
        ...
        if callable(send_snapshot_to) and isinstance(players, dict) and players:
            ...
            for player_id in list(players.keys()):
                ...
                try:
                    ...
                    continue
                if not last_sig:
                    last_sig = self._scene_signature(snapshot.get("scene", {}))
            self._last_host_scene_signature = last_sig
            return
        snapshot = self._build_online_snapshot()
        self._last_host_scene_signature = self._scene_signature(snapshot.get("scene", {}))
    ...
    def _on_host_command_received(self, player_id: str, message: dict) -> None:
        ...
        if action == "ping":
            try:
                ...
                return
            dungeon_id = str(payload.get("dungeon_id") or self._players_dungeon_id or "")
            self._show_network_ping(x, y, dungeon_id)
        ...
        *,
        ...
        if str(item_data.get("owner_player_id") or "").strip() != player_id:
            self._host_controller.send_command_result(
                player_id,
                ok=False,
                message="Entity owned by different player",
                request_id=request_id,
            )
            return
    ...
    def _remove_selected_items_from_inventory_payload(
        ...
        if isinstance(raw_equipment, dict):
            for key, value in raw_equipment.items():
                ...
                equipment[slot_id] = clean_value or None
        missing: list[str] = []
        for selected in selected_items:
            ...
            if source == "equipment":
                ...
                removed = False
                if source_slot and str(equipment.get(source_slot) or "").strip() == clean_item_id:
                    equipment[source_slot] = None
                    removed = True
                if not removed:
                    for slot_id, value in equipment.items():
                        if str(value or "").strip() != clean_item_id:
                            continue
                        equipment[slot_id] = None
                        removed = True
                        break
            ...
            if isinstance(source_index, int):
                if 0 <= source_index < len(remaining_inventory):
                    if str(remaining_inventory[source_index] or "").strip() == clean_item_id:
                        remaining_inventory.pop(source_index)
                        removed = True
            if not removed:
                try:
                    matched_index = remaining_inventory.index(clean_item_id)
                except ValueError:
                    matched_index = -1
                if matched_index >= 0:
    ...
    def _handle_host_add_loot_from_inventory(
        ...
        *,
        ...
        if not sheet_id or not isinstance(selected_items, list):
            ...
            return
        if not self._player_has_linked_sheet(player_id, sheet_id):
            self._host_controller.send_command_result(
                player_id,
                ok=False,
                message="Transfer target character is not linked to your entities.",
                request_id=request_id,
            )
            return
        ...
        for item in parsed_items:
            entry_payload = {
                ...
                "type": "item",
                "item_id": str(item.get("item_id") or "").strip(),
                "title": str(item.get("title") or item.get("item_id") or "Item"),
                "path": str(item.get("path") or item.get("item_id") or ""),
            }
            item_document = item.get("item_document")
            if isinstance(item_document, dict):
                entry_payload["item_document"] = item_document
            else:
                source_path = Path(str(item.get("path") or item.get("item_id") or "")).expanduser()
                if source_path.exists():
                    source_doc = self._loot_pool_item_document_from_path(source_path)
                    if isinstance(source_doc, dict):
                        entry_payload["item_document"] = source_doc
        ...
        *,
        ...
        if not self._player_has_linked_sheet(player_id, sheet_id):
            self._host_controller.send_command_result(
                player_id,
                ok=False,
                message="Claim target character is not linked to your entities.",
                request_id=request_id,
            )
            return
        ...
        for entry in self._session_loot_pool:
            ...
            if entry_id in selected_set:
                existing_claim = self._loot_claim_entry_reservations.get(entry_id)
                if existing_claim:
                    self._host_controller.send_command_result(
                        player_id,
                        ok=False,
        ...
        claim_entry_ids = [str(entry.get("entry_id") or "") for entry in claimed_entries]
        ...
        if applied:
            ...
            if entry_ids:
                self._session_loot_pool = [
                    entry
                    for entry in self._session_loot_pool
                    if str(entry.get("entry_id") or "").strip() not in entry_ids
                ]
                self._refresh_loot_pool_list()
    ...
    def _release_loot_claim_reservation(self, claim_id: str) -> None:
    ...
    def _drop_invalid_loot_claim_reservations(self) -> None:
        ...
        for claim_id, claim in self._loot_claim_reservations.items():
            ...
            if not claim_entry_ids:
                ...
                continue
            if not claim_entry_ids.issubset(current_entry_ids):
                release_claim_ids.append(claim_id)
    ...
    def _push_local_character_overrides_to_host(
        self,
        *,
        prefer_sheet_ids: set[str] | None = None,
    ) -> int:
        if self._online_mode != ONLINE_MODE_PLAYER:
            return 0
        if self._client_controller is None:
        ...
        if not local_player_id:
            return 0
        preferred_sheets: set[str] | None = None
        if isinstance(prefer_sheet_ids, set):
            cleaned = {str(sheet_id or "").strip() for sheet_id in prefer_sheet_ids}
            preferred_sheets = {sheet_id for sheet_id in cleaned if sheet_id}
            if not preferred_sheets:
                return 0

        cached_sheet_payloads: dict[str, dict] = {}
        sent_count = 0
        for dungeon in self._dungeons:
            ...
            for item_data in items:
                ...
                if not entity_id or not sheet_id:
                    continue
                if preferred_sheets is not None and sheet_id not in preferred_sheets:
                    continue
                payload = cached_sheet_payloads.get(sheet_id)
                if payload is None:
                    payload = self._resolve_local_sheet_sync_payload(sheet_id)
                    if payload is None:
                        continue
                    cached_sheet_payloads[sheet_id] = payload

                host_payload = {
                    "entity_id": entity_id,
                    "sheet_id": str(payload.get("sheet_id") or sheet_id),
                    "sheet_name": str(payload.get("sheet_name") or sheet_id),
                    "character_id": str(payload.get("character_id") or ""),
                    "inventory": normalize_inventory_payload(payload.get("inventory")),
                    "stats": dict(payload.get("stats") or {}),
                    "dungeon_id": dungeon_id or str(self._active_dungeon_id or ""),
                }
                fingerprint_source = {
                    "entity_id": host_payload["entity_id"],
                    "sheet_id": host_payload["sheet_id"],
                    "character_id": host_payload["character_id"],
                    "inventory": host_payload["inventory"],
                    "stats": host_payload["stats"],
                }
                try:
                ...
                sent_key = f"{host_payload['dungeon_id']}::{entity_id}"
    ...
    def _on_client_snapshot_received(self, snapshot: dict) -> None:
        def _sync_owned_sheet_inventories_from_snapshot() -> None:
            ...
            if not local_player_id:
                return
            inventory_by_sheet: dict[str, dict] = {}
            local_priority_sheets: set[str] = set()
            for dungeon in self._dungeons:
                state = dungeon.get("state")
                if not isinstance(state, dict):
                    continue
                ...
                for item_data in items:
                    ...
                    if self._local_character_sheet_exists(sheet_id):
                        local_priority_sheets.add(sheet_id)
                        continue
                    sheet_name = str(item_data.get("linked_sheet_name") or sheet_id).strip() or sheet_id
                    linked_inventory = item_data.get("linked_inventory")
                    if not isinstance(linked_inventory, dict):
                        continue
                    inventory_by_sheet.setdefault(
                        sheet_id,
                        {
                            "sheet_name": sheet_name,
                            "inventory": normalize_inventory_payload(linked_inventory),
                        },
                    )
            if local_priority_sheets:
                sent_overrides = self._push_local_character_overrides_to_host(
                    prefer_sheet_ids=local_priority_sheets
                )
                if sent_overrides > 0:
                    self._append_server_log(
                        "[INFO] Prioritized local linked character and synced update(s) to host."
                    )
            for sheet_id, sync_payload in inventory_by_sheet.items():
                inventory_payload = sync_payload.get("inventory", {})
                sheet_name = str(sync_payload.get("sheet_name") or sheet_id)
        ...
        def _run_post_snapshot_character_sync() -> None:
            sent_overrides = 0
            if self._pending_join_character_override_sync:
                self._pending_join_character_override_sync = False
                sent_overrides = self._push_local_character_overrides_to_host()
                if sent_overrides > 0:
                    # Intentional: on initial join we push the player's local linked
                    # character state to host so DM and host storage are overwritten
                    # with the player's latest character edits.
                    self._append_server_log(
                        f"[INFO] Synced {sent_overrides} linked character update(s) to host."
                    )
            if sent_overrides == 0:
    ...
    def _on_client_command_result(self, result: dict) -> None:
        if result.get("ok"):
            ...
            if isinstance(data, dict) and str(data.get("action") or "") == "add_loot_from_inventory":
                sheet_id = str(data.get("sheet_id") or "").strip()
                inventory_payload = data.get("inventory")
                if sheet_id and isinstance(inventory_payload, dict):
                    ok, message = self._sync_local_sheet_inventory_from_host(
                        sheet_id,
                        inventory_payload,
    ...
    def _load_or_create_local_profile(self) -> dict:
        default_profile = {
            "version": 1,
            "player_id": _generate_probabilistic_unique_id("player"),
            ...
            if path.exists():
                ...
                if isinstance(payload, dict):
                    ...
                    if not str(merged.get("player_id") or "").strip():
                        merged["player_id"] = _generate_probabilistic_unique_id("player")
    ...
    def _refresh_dungeon_list(self, preserve_selection: bool = True) -> None:
        current_id = self._active_dungeon_id if preserve_selection else None
    ...
    def _autosave_collection_path(self) -> Path:
        if self._collection_path is not None:
            ...
            if stem.endswith(COLLECTION_AUTOSAVE_SUFFIX):
                ...
                filename = f"{stem}{COLLECTION_AUTOSAVE_SUFFIX}{self._collection_path.suffix or '.json'}"
        ...
        return base_dir / f"{stem}{COLLECTION_AUTOSAVE_SUFFIX}.json"
    ...
    def _materialize_state_icons_for_save(self, state: dict, target_path: Path) -> dict:
        if not isinstance(state, dict):
            return state
        items = state.get("items")
        if not isinstance(items, list):
            return state
        icons_dir = self._collection_working_icon_dir(target_path)
        for item_data in items:
            if not isinstance(item_data, dict):
                continue
            if item_data.get("type") != "entity":
            ...
            try:
                ...
                saved_icon = icons_dir / filename
                if not saved_icon.exists():
                    saved_icon.write_bytes(raw)
            except Exception:
                continue
            item_data["icon_path"] = str(saved_icon)
        return state

    def _build_collection_payload(self, target_path: Path | None = None) -> dict:
        self._save_active_dungeon_state()
        dungeons_payload = []
        for dungeon in self._dungeons:
            ...
            if target_path is not None:
                state_for_save = self._materialize_state_icons_for_save(state_for_save, target_path)
            dungeons_payload.append(
                {
                    "id": dungeon["id"],
                    "name": dungeon["name"],
    ...
    def _save_collection_as(self) -> bool:
        ...
        default_name = _sanitize_filename(self._collection_name, "dungeon_collection") + ".json"
    ...
    def _save_collection_to_path(self, path: Path, *, commit_as_primary: bool = True) -> bool:
        payload = self._build_collection_payload(path)
        try:
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
        except Exception as exc:
