...
class DungeonAppletWidget(QWidget):
    ...
    def _dispatch_player_command_with_request_id(
        ...
        *,
        ...
        sent = self._client_controller.send_command(
            action,
            payload,
    ...
    def _on_client_hello_ack(self, player_id: str) -> None:
        ...
        self._append_chat_message(
            "System",
            f"Joined as {self._local_player_name}. Waiting for host snapshot...",
            True,
        )
        self._apply_online_permissions()
    ...
    def _handle_host_sync_character_inventory(
        ...
        *,
        ...
        if not character_id:
            ...
            return
        if not self._player_owns_linked_character(player_id, sheet_id, character_id):
            self._host_controller.send_command_result(
                player_id,
                ok=False,
                message="Character sync target is not linked to one of your owned entities.",
                request_id=request_id,
            )
            return
        ...
        *,
        ...
        for dungeon in self._dungeons:
            ...
            for item_data in items:
                ...
                character_id = str(item_data.get("linked_character_id") or "").strip()
                linked_sheet_name = (
                    str(item_data.get("linked_sheet_name") or sheet_id).strip() or sheet_id
                )
                if character_id:
                    break
            if character_id:
    ...
    def _handle_host_finalize_loot_claim(
        ...
        *,
        ...
        restored_any = self._restore_claimed_loot_entries(claim)
        self._release_loot_claim_reservation(claim_id)
    ...
    def _linked_item_document_by_id(self, item_id: str) -> dict | None:
        ...
        if not clean_item_id:
            return None
        candidate = Path(clean_item_id).expanduser()
        if candidate.exists():
            document = load_item_document(candidate)
            if isinstance(document, dict):
                return document
        ...
        for item_path in list_item_file_paths(root):
            ...
            if candidate_id != clean_item_id:
                continue
            document = load_item_document(item_path)
            if isinstance(document, dict):
                return document
        return None
    ...
    def _canonicalize_linked_inventory_payload(
        ...
        *,
        ...
        for item_id in referenced_item_ids:
            ...
            chosen_document = existing_document or library_document
    ...
    def _resolve_local_sheet_sync_payload(self, character_id: str) -> dict | None:
        ...
        try:
            from player_sheets import (
                archive_bytes_for_character_id,
        ...
        archive_bytes = archive_bytes_for_character_id(clean_character)
        archive_b64 = (
    ...
    def _replay_host_link_conflict_response(
        ...
        *,
        ...
        if str(cached.get("signature") or "") != request_signature:
            return False
        created_monotonic = float(cached.get("created_monotonic") or 0.0)
        if created_monotonic <= 0.0 or (time.monotonic() - created_monotonic) > 10.0:
            self._host_link_conflict_request_cache.pop(cache_key, None)
            return False
        self._host_controller.send_command_result(
            player_id,
        ...
        for dungeon in self._dungeons:
            ...
            for item_data in items:
                ...
                fingerprint_source = {
                    ...
                    "archive_b64": host_payload["archive_b64"],
                }
                try:
        ...
        def _sync_owned_sheet_inventories_from_snapshot() -> None:
            ...
            for dungeon in self._dungeons:
                ...
                for item_data in items:
                    ...
                    elif character_id in self._approved_host_inventory_sync_characters:
                    ...
                    if conflict_key not in conflicts_by_key:
                        conflicts_by_key[conflict_key] = conflict_payload
    ...
    def _on_client_command_result(self, result: dict) -> None:
        ...
        if action == "claim_loot_finalize":
            claim_id = str(data.get("claim_id") or "").strip()
            rollback = self._pending_loot_claim_rollbacks.pop(claim_id, None) if claim_id else None
            if isinstance(rollback, dict):
                ok, rollback_message = self._restore_sheet_inventory_snapshot(
                    str(rollback.get("sheet_id") or ""),
                    dict(rollback.get("inventory") or {}),
                )
                if not ok:
                    self._append_server_log(
    ...
    def _on_external_character_inventory_saved(self, sheet_id: str, inventory_payload: dict) -> None:
        ...
        self._apply_inventory_sync_to_linked_entities(
            ...
            archive_b64=str(sync_payload.get("archive_b64") or "") if isinstance(sync_payload, dict) else "",
        )
        if self._online_mode == ONLINE_MODE_DM_HOST:
            self._broadcast_snapshot_if_host()
